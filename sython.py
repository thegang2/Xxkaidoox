...
@sython.on(events.NewMessage(outgoing=True, pattern=r"\.تجميع دعمكم"))
async def _(event):
        await event.edit("حسنا, تأكد من انك مشترك ب قنوات الاشتراك الاجباري لتجنب الأخطأء")
        channel_entity = await sython.get_entity(bot_usernamee)
        await sython.send_message('@DamKombot', 'جاري التجميع بواسطة | SOMY TEAM')
        channel_entity = await sython.get_entity(bot_usernamee)
        await sython.send_message('@DamKombot', '/start')
        await asyncio.sleep(5)
        msg0 = await sython.get_messages('@DamKombot', limit=1)
        await msg0[0].click(2)
        await asyncio.sleep(5)
        msg1 = await sython.get_messages('@DamKombot', limit=1)
        await msg1[0].click(0)
        ...
        for i in range(100):
            ...
            try:
                try:
                    ...
                    bott = url.split('/')[-1]
                    await sython(ImportChatInviteRequest(bott))
                msg2 = await sython.get_messages('@DamKombot', limit=1)
                await msg2[0].click(text='تحقق')
                chs += 1
